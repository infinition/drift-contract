...
\item A depth result: local Adam degrades from 44.4 to 31.3 percent as depth goes 12 to 48 even with per-depth re-tuning; spectral updates lose 3.3 percentage points with a single unchanged setting.
...
% Numbers are validation-selected (analyze_val.py, phase D complete). Refresh at DONE.
\begin{table}[t]
\centering
\caption{Depth scaling at width 128, validation-selected setting per arm, test
accuracy. Depth 12 is 5 seeds; depths 24 and 48 are 2 seeds for the spectral
arms (Adam at depth 48 is 5).}
\label{tab:depth}
\begin{tabular}{lccc}
\toprule
...
% Numbers are validation-selected (analyze_val.py). At DONE, refresh: width 2048
% reaches 5 seeds only if added to phase S, and a few Adam cells gain seeds.
\begin{table}[t]
\centering
...
MLP classifiers on CIFAR-scale data; linear local heads; 12 fixed epochs (no schedule study, so part of the gap may reflect optimization speed rather than converged performance); per-layer drift is bounded but total network drift grows with depth (0.20 to 0.55 from depth 12 to 48 at fixed $\eps$, suggesting depth-aware budgets); Newton-Schulz overhead is 10 to 15 percent at these sizes. Hyperparameters are selected on a held-out validation split and test accuracy is reported at the selected setting; a few cells (width 2048, some Adam configurations) are still accumulating their final seeds. The drift bound is conditional (Section 2): it covers weight-induced drift at the current input, not propagated or off-distribution drift. The negative normalization result bounds the claim: spectral local updates matter most in unnormalized or normalization-averse settings (analog hardware, minimal-memory edge training). $\eps$ is scale-invariant but not domain-invariant (0.01 on our synthetic task, 0.003 on CIFAR).
